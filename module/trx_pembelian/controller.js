        ...
        try { 
            ...
            if(cekStatus[0].status_persetujuan_txp == 3){
                ...
                if(status_persetujuan_txp == 3){
                    ...
                    let akunHutang = await sq.query(`select c6.*,gl.sisa_saldo from coa6 c6 join coa5 c5 on c5.id = c6.coa5_id left join general_ledger gl on gl.akun_id = c6.id and gl.status = 1 where c6."deletedAt" isnull and c5.company_id = '${cekStatus[0].company_id}' and c6.kode_coa6 = '2.1.7.1.01.0001' order by gl.tanggal_persetujuan desc limit 1`,s);
                    let akunBarang = await sq.query(`select gl.sisa_saldo from general_ledger gl where gl."deletedAt" isnull and gl.akun_id = '${cekStatus[0].coa6_id}' order by gl.tanggal_persetujuan desc`,s);

                    let saldoBarang = akunBarang.length>0?akunBarang[0].sisa_saldo+cekStatus[0].harga_total_txp:cekStatus[0].harga_total_txp
                    let saldoHutang = !akunHutang[0].sisa_saldo?cekStatus[0].harga_total_txp:akunHutang[0].sisa_saldo+cekStatus[0].harga_total_txp

                    let barang = {id:uuid_v4(),sisa_saldo:saldoBarang,tanggal_transaksi:cekStatus[0].p.tanggal_pembelian,penambahan:harga_total_txp,pembelian_id,akun_id:cekStatus[0].coa6_id,akun_pasangan_id:akunHutang[0].id,nama:"barang"}
                    let hutang = {id:uuid_v4(),sisa_saldo:saldoHutang,tanggal_transaksi:tanggal_pembelian,penambahan:harga_total_txp,pembelian_id,akun_id:akunHutang[0].id,akun_pasangan_id:cekStatus[0].coa6_id,nama:"hutang"}
